...
const AdminDashboard = () => {
  ...
  const resetCurrentToken = async () => {
    if (
      window.confirm("Are you sure you want to reset the current token to 0?")
    ) {
      setUpdating(true);
      try {
        await updateClinicMeta({
  ...
  const deleteAllTokens = async () => {
    if (
      window.confirm(
        "Are you sure you want to delete all tokens for today? This action cannot be undone."
      )
    ) {
      setUpdating(true);
      try {
  ...
  const upcomingTokens = tokens.filter(
    (token) => token.token_number > clinicMeta.current_token
  );
        ...
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-white rounded-xl shadow-sm p-6 h-40"
              ></div>
            ))}
          </div>
        </div>
                  ...
                  <div className="space-y-4">
                    {upcomingTokens.map((token) => (
                      <TokenCard
                        ...
                        token={token}
                        isCurrent={
                          token.token_number === clinicMeta.current_token + 1
                        }
                      />
                    ))}
                  </div>
